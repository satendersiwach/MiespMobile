...
//        EditText e1 = findViewById(R.id.editText);
        ...
        // ***************************************************//
        {
            if (mReaderManager != null)
            {
                try{
                    ...
                    BcReaderType myReaderType =  mReaderManager.GetReaderType();
                    Toast.makeText(this, "Reader type fetched = "+myReaderType, Toast.LENGTH_SHORT).show();
                    Log.d("MainActivity", "Reader Type: " + myReaderType);
                } catch (java.lang.Exception e) {
                    Log.e("Exception", e.getMessage());
                    Log.e("MainActivity", "Error in ExeSampleCode: " + e.getMessage(), e);
                    Toast.makeText(this, "Error in ExeSampleCode: " + e.getMessage(), Toast.LENGTH_SHORT).show();
                }
    ...
    private final BroadcastReceiver myDataReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            // Handle software trigger action
            if (intent.getAction().equals(GeneralString.Intent_SOFTTRIGGER_DATA)) {

                // Extract barcode data from the intent
                String data = intent.getStringExtra(GeneralString.BcReaderData);

                // Handle the decoded data (e.g., log it, send it to another method, etc.)
                // For now, we'll just log it
                Log.d("ScannerData", "Decoded Data: " + data);

            } else if (intent.getAction().equals(GeneralString.Intent_PASS_TO_APP)) {
                // Handle the scenario where keyboard emulation is disabled
                String data = intent.getStringExtra(GeneralString.BcReaderData);

                // Log the decoded data and track the decode count
                mDecodeCount++;
                Log.d("ScannerData", "[" + mDecodeCount + "] Decoded Data: " + data);

            } else if (intent.getAction().equals(GeneralString.Intent_READERSERVICE_CONNECTED)) {
                // Ensure the app is connected to the barcode reader service
                BcReaderType myReaderType = mReaderManager.GetReaderType();

                // Log the reader type (no UI interaction needed)
                Log.d("ScannerData", "Reader Type: " + myReaderType.toString());

                // Get and set reader output configuration (this part can be customized based on your needs)
                ReaderOutputConfiguration settings = new ReaderOutputConfiguration();
                mReaderManager.Get_ReaderOutputConfiguration(settings);
        ...
        new MethodChannel(flutterEngine.getDartExecutor().getBinaryMessenger(), CHANNEL)
                .setMethodCallHandler((call, result) -> {
                    if (call.method.equals("configureRFID")) {
                        configureRFID();
                        result.success("RFID configured successfully");
                    } else {
